...
def ssh_key_generator(path_generator):
    ...
    def generator():
        ...
        subprocess.run(
            ["ssh-keygen", "-N", "", "-t", "rsa", "-b", "4096", "-f", key],
            check=True,
            capture_output=True,
        )
        return key
...
def path_generator(random_string_generator):
    ...
    def generator(
        name_prefix, base_dir="/tmp", suffix_length=5, mkdir=False, touch=False, cleanup=False
    ):
...
def random_backend_generator():
    ...
    def generator(backend_type=None):
        backend_classes = backendbase.BackendBase.backend_classes()
        backend_class = (
            random.choice(backend_classes)
            if backend_type is None
            else next(cls for cls in backend_classes if cls.name() == backend_type)
        )
...
@pytest.fixture
def random_backup_generator(
    random_timeframes_generator,
    btrfs_fs_generator,
    sshtarget_generator,
    random_backend_generator,
    path_generator,
    ...
    def generator(backend_type=None, backup_type=None, num_timeframes=3, backend=None):
        backup_type = (
            backup_type
            if backup_type is not None
            else random.choice(["local_to_local", "local_to_remote", "remote_to_local"])
        )
        timeframes = random_timeframes_generator(num=num_timeframes)

        if backend is None:
            backend = random_backend_generator(backend_type)
        if backend_type is None:
            backend_type = backend.name()
        elif backend_type != backend.name():
            raise ValueError(f"backend type '{backend_type}' does not match '{backend.name()}'")
        ...
        if backend_type == "btrfs":
            ...
            raise NameError(f"Unknown backend type '{backend_type}'")
...
def random_timeframe_generator(
    random_timeframe_times_generator,
    random_timeframe_minutes_generator,
    random_timeframe_weekdays_generator,
    random_timeframe_monthdays_generator,
    ...
    def generator(
        ...
        weekdays = (
            random_timeframe_weekdays_generator(random.randint(1, 3))
            if weekdays is None
            else weekdays
        )
        monthdays = (
            random_timeframe_monthdays_generator(random.randint(1, 3))
            if monthdays is None
            else monthdays
        )
        yeardays = (
            random_timeframe_yeardays_generator(random.randint(1, 3))
            if yeardays is None
            else yeardays
        )
...
def btrfs_fs_generator(path_generator, loopback_generator):
    ...
    def generator():
        ...
        subprocess.run(
            ["btrfs", "subvolume", "create", f"{mountpoint}/@"], check=True, capture_output=True
        )
        subprocess.run(["umount", mountpoint], check=True, capture_output=True)
        subprocess.run(
            ["mount", loop, "-o", "rw,noatime,subvol=@", mountpoint],
            check=True,
            capture_output=True,
        )
        return mountpoint
...
def valid_raw_config_generator(random_backup_generator):
    ...
    def generator(num_backups=3):
        ...
        for backup in backups:
            ...
            if backend.backup_type == "local_to_local":
                backup_settings["src_dir"] = str(backend.src_dir)
                backup_settings["dst_dir"] = str(backend.dst_dir)
            elif backend.backup_type == "local_to_remote":
                backup_settings["src_dir"] = str(backend.src_dir)
                backup_settings["dst_dir"] = backend.dst_dir.spec
